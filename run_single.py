...
"""
    ...
    if unicode_enc:
        ...
        message = decode_arithmetic(
            model, enc, message_str, message_ctx, precision=40, topk=60000, device="cpu"
        )
    ...
    if mode == "arithmetic":
        ...
        out, nll, kl, Hq = sample(
            model, enc, sample_tokens, context_tokens, temperature=temp, topk=topk
        )
        words_per_bit = 1
    text = enc.decode(out)
    ...
    if mode != "sample":
        if mode == "arithmetic":
            ...
            message_rec = decode_huffman(
                model, enc, text, context_tokens, block_size, device="cpu"
            )
        elif mode == "bins":
            message_rec = decode_block(
                model,
                enc,
                text,
                context_tokens,
